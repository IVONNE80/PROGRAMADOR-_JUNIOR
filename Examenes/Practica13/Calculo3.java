...
public class Calculo3 extends Frame implements ActionListener {
    ...
    public Calculo3(String title, Integer width, Integer height) {
        setTitle(title);
    ...
    public void ejecutaOrden(String orden) {
        if (ordenNumerica(orden)) {
            if (((String) calculo.getText()).equals("0"))
                calculo.setText(orden);
            else
                calculo.setText(calculo.getText() + orden);
    ...
    public int ejecutarOp() {
        ...
        for (int i = 1; i <= c; i++) {
            if (operaciones[i - 1] == 0)
                resultado += cifras[i];
            else if (operaciones[i - 1] == 1)
                resultado -= cifras[i];
            else if (operaciones[i - 1] == 2)
                resultado *= cifras[i];
            else if (operaciones[i - 1] == 3)
                resultado /= cifras[i];
        }
        return resultado;
    }
    ...
    public static void main(String args[]) {
        Calculo3 c = new Calculo3("Calculadora", 300, 400);
    }
